...
TYPES = (
    ...
    ('execution.state_changed', 'execution', 'user', False, 'an execution moved state'),
    ...
    ('approval.state_changed', 'approval', 'user', False,
     'an approval moved state (approved, rejected, expired, superseded, consumed)'),
    ('policy_rule.created', 'policy_rule', 'user', False,
     'a policy rule (or a revision of one) was written (payload: before, after)'),
    ('policy_rule.retired', 'policy_rule', 'user', False,
     'a policy rule was retired (payload: before, replaced_by)'),
    ('policy_decision.created', 'policy_decision', 'system', False,
     'a policy evaluation was recorded (payload: stage, decision, outcome, mission, plan, task)'),
    ('verification.created', 'verification', 'user', False, 'a task or criterion is checked'),
    ...
    ('project.created', 'project', 'user', False, 'a project and its repositories were registered'),
    ('repository.registered', 'repository', 'system', False, 'a repository joined a project'),
    ('repository_inspection.state_changed', 'repository_inspection', 'system', False,
     'an inspection moved state (scheduled, running, completed, failed)'),
    ('architecture.state_changed', 'repository', 'user', False,
     'a repository\'s architecture assessment moved (payload: from, to, trigger, reason)'),
    ('knowledge_item.created', 'knowledge_item', 'user', False, 'a knowledge item was recorded'),
    ('knowledge_item.state_changed', 'knowledge_item', 'user', False,
     'a knowledge item moved state (confirmed, superseded, retracted)'),
    ('knowledge_item.purged', 'knowledge_item', 'system', False,
     'the content of a forgotten knowledge item was blanked; its tombstone keeps provenance'),
    ('user.digest_acked', 'user', 'system', False, 'the digest was acknowledged up to a seq'),
    ('user.updated', 'user', 'user', False,
     "the owner's settings changed (payload: the fields, e.g. autonomy_profile)"),
    ('relation.created', 'relation', 'system', False,
     'a relation between two world or knowledge objects was recorded'),
    ('route.decided', 'route_decision', 'system', False,
     'a resource was chosen (or none survived) for a task, a review or an own call'),
    ('archeus_call.ended', 'route_decision', 'system', False,
     'one of Archeus\'s own calls ended (payload: outcome state, items, usage)'),
    ('provider_terms.decided', 'provider_terms', 'user', False,
     'the user answered the provider-terms question for a harness (ADR-0021)'),
    ...
    ('account.registered', 'account', 'user', False,
     'an account of a harness was registered (payload: harness_id, label, auth_kind)'),
    ('account_health.state_changed', 'account', 'user', False,
     'an account moved along its health machine (payload: from, to, trigger, reason)'),
    ('resource_policy.updated', 'resource_policy', 'user', False,
     "an account's priority, allocation, budgets or restrictions changed (payload: fields)"),
    ('usage.observed', 'account', 'system', False,
     'a provider usage window was observed for an account (payload: window, pct, source)'),
