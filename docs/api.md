...
| `/api/logs` | `api_logs` | archeus's own event log — what it did and what failed, newest first. |
| `/api/loop-md` | `api_loop_md_get` | — |
| `/api/loops` | `api_loops` | Loops archeus started, with live state read off the process and the |
...
| `/api/plan/last` | `api_plan_last` | Read back <project>/.archeus/plan-latest.md, split into the task |
...
| `/api/versions` | `api_versions` | archeus and the installed Claude Code against what has been released, |
