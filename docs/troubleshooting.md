...
| Window closes instantly with an error | Check `%TEMP%\archeus_crash.log` — the crash handler writes the traceback there |
...
| Wrong account / want a second account | Set **Config dir** in **⚙ Settings** to that account's `CLAUDE_CONFIG_DIR` (e.g. `~/.claude-work`). Drives both session browsing and the env handed to `claude` at launch. Blank = default `~/.claude`. Restart archeus to apply. See [Multiple accounts](accounts.md) for running two at once. |
| Settings location | `~/.claude/archeus.json` — safe to edit by hand or delete to reset (always read from `~/.claude`, independent of Config dir) |
| Usage stats look stale | Delete `~/.claude/archeus-stats-cache.json` — it rebuilds on the next scan |
...
| Something failed and said nothing | Open **⚙ Logs** (TUI) or the **Logs** page (GUI). Background jobs, the auto-memory scheduler and archeus's own Claude calls all record their failures there |
| "No output from Claude" when generating an agent / skill / CLAUDE.md | Usually the account's limit. archeus now says so by name and offers another account — see [Rate limits and a second account](tui.md#rate-limits-and-a-second-account) |
...
archeus looks for the Claude Code CLI on your `PATH` and at `~/.local/bin/`. This screen
means neither had it.

- Install [Claude Code](https://docs.anthropic.com/claude-code) if you have not, then
  reopen archeus — the check runs at startup, not once.
- If it *is* installed, the shell that launched archeus has a different `PATH` than the
...
- On Windows the executable is often `claude.cmd` or a `claude.exe` shim in
  `%APPDATA%\npm`. Point at whichever `where claude` prints; archeus runs it as-is.

archeus never installs or updates Claude Code, and never writes your credentials.
...
Almost always the account's limit. archeus names the account and offers you another one.
...
browser — because the cause was the page, not the compositor. If archeus flickered while
you were working in *another* app, this was it.

While the window is not focused, archeus now:
...
Usually the folder moved or was deleted. archeus lists the projects Claude Code has
recorded and does not invent them, so launch a session in the new location once and it
comes back.

The other cause is a path archeus could not resolve. It reads the real `cwd` out of each
...
For usage stats, delete `~/.claude/archeus-stats-cache.json` and it rebuilds on the next
scan.
...
| File | What it holds |
|------|---------------|
| `~/.claude/archeus-events.jsonl` | **Start here.** archeus's own events — failed Claude calls with what `claude` actually printed, job crashes, scheduler passes, quarantined state files. Capped at 256 KB. Shown by the Logs screen in both interfaces |
| `%TEMP%\archeus_crash.log` | The traceback when the window closes instantly |
