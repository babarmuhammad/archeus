...
A project in archeus is a folder Claude Code has a session for, and it carries state of
...
- **Context-loss insurance** — after every session a 5-line summary (goal + files touched) is appended to `.archeus/session-log.md`, so the next session can recall what happened even after `/compact` wiped the context. Local, free.
...
archeus tracks the **provenance and freshness** of the context it generates. After
scaffold, AI-analyze, or launch, it writes `<project>/.archeus/workspace-manifest.json`
(falling back to the encoded `~/.claude/projects/<encoded>/.archeus/` folder if the working
...
`CLAUDE.md` has two halves and archeus owns exactly one of them: it rewrites the AUTOGEN,
...
snapshotted under `.archeus/snapshots/`, so the workspace screen (`w`) lists recent changes
