---
description: >-
  archeus's desktop GUI — full feature parity with the terminal UI, served locally with
...
- **Sidebar** — five sections, each a set of sub-tabs across the top of the page:
  **Context** (global CLAUDE.md, output styles, MCP servers), **Library** (agents, skills,
  hooks, plugins), **Activity** (usage, loops, logs), **Harnesses** (every CLI archeus
  drives, its accounts and Claude Code's own record of itself) and **Settings** (launch,
  appearance, paths & limits, models, updates). Search is the box on the dashboard and
  `Ctrl+K`; Help is the `?` at the foot of the sidebar. The same five names, in the same
  order, are the terminal UI's five submenus and the five sections of this documentation.
- **Project tabs** — four tabs, each grouping the pages that answer one question:
  **Session**, **Context** (memory, CLAUDE.md, audit), **Project** (tools, usage, repos)
  and **Actions** (plan → execute, code review). Each group opens on the tab it is opened
  *for*, and remembers which one you were last on. The page you are on names itself in one
  line under the tabs, from the same sentence the help page renders. The three that carry
  the most:
    - **Memory** — *what archeus knows, and what it costs.* It opens on what it knows, then what it learned, then the price. An inventory of every memory artifact the project has, each with what writes it, whether it reaches a session **always / lazily / per prompt / never**, its size, and its action — plus the sentence the UI never used to say: *~N tok read on every turn, ~M tok across the rule files that cost nothing until Claude opens a matching path.* Then the live state: what the last cycle extracted and what it spent, **total spend to date**, what is queued, what eviction dropped, which entities recall reinforces most, lessons with **how many sessions each is from being dropped**, and workspace health as per-check rows with the button that fixes each one. Build / ask / recall preview / lessons review, with **live scan progress** and the outcome of the last run.
    - **CLAUDE.md** — *the file, block by block.* It opens on **Your prose**: an editor holding only the half of the file you wrote, with a Save button. Everything archeus generates is left out of that box and put back untouched on save, so there is nothing there you can break, and `KEEP` fences travel with your text. Below it: your prose, KEEP-fenced regions, AUTOGEN, SESSIONS and the MEMORY digest, each with its token cost and the one button that regenerates just that block; the memory files map with broken `@import`s flagged; and every version archeus replaced, with diff and restore.
    - **Audit** — *what one turn costs across every surface at once*, project-scoped and account-scoped together (global CLAUDE.md per account, system-prompt file, SessionStart hooks, MCP schemas), plus deny rules for token-heavy paths, project health with the button that fixes each issue, and Claude Code's own record of the project.
    - Also: **Tools** in two columns — what the project launches with (its agents picker, `--add-dir` directories, extra PATH entries) on the left, what it has been doing (most-run commands, what to work on) on the right — plus Usage, Repos, **Plan → Execute** (plan model + effort, execute via Anthropic or free OmniRoute, full explanation inline) and the architecture **Graph**, which opens in its own window from the tab strip.
...
- **Global CLAUDE.md** — the first tab of **Context**: the instructions read in every
  session on an account, its `loop.md` sibling and the cross-project conventions worth
  promoting into it.
- **Output styles** — by scope, with the active one and the file that pins it named, plus
  four archeus starters to copy (Terse, Reviewer, Pair, Ship).
- **Logs** — what archeus itself did and why it failed: its own headless Claude calls,
...
Four ship — `anime`, `cyber`, `deck` and `graph`, the last an homage to archeus's own
