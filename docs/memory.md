...
- **Auto-refresh that actually converges.** Turn it on per project (memory hub → `o`, or the checkbox on the GUI's memory tab — one flag, honoured by both interfaces and by the background worker) and **memory does not go stale while it is on**:
    - Each cycle extracts at most `auto_cap` modules and leaves the rest queued, and the next pass comes at the interval you configured — never sooner. A 45-second catch-up loop used to fire whenever work was owed, which on a repo with more changed modules than one cycle can afford spent most of a daily limit inside an hour, and on a rate-limited account retried the same six failures forever. The cap decides how much a pass may spend; the interval decides how often. To drain a backlog deliberately, use **Build modules** below. It used to do *nothing at all* once more than six modules had changed — the harder you worked, the less it updated.
...
- **Build the backlog on purpose** (Settings → Updates → *Build modules*) — the schedule above spends a little, often, and only on the projects you checked. This spends the idle time instead: one project after another across **every account**, stalest first (units already owed, then never-built, then oldest graph), until nothing is left to build. It runs only because you pressed the button. It waits while one of your own jobs is running or a Claude Code session is live — measured the same way the dashboard measures it, so archeus's own headless calls do not read as you working — and it drops an account for the rest of the run if Claude says that one is rate-limited. Each project is capped exactly as a scheduled pass is and the sweep repeats, so one big repo cannot starve the others. Cancel it on its banner, which follows you from page to page.
    - It is also what made per-account building correct at all: memory's Claude call passed no environment, so every build spent whichever account the archeus *process* resolved rather than the one the project belongs to — the wrong quota, and the wrong attribution in that project's cost history.
