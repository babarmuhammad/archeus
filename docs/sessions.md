...
- **Self-update** — archeus checks PyPI for a newer release and says so in the banner. `⚙ Updates` (and the GUI's Plugins page) offers the upgrade; `Settings → Updates` switches between *tell me*, *install on quit* and *off*. The install runs in a background worker with no window of its own, **while archeus stays open**: pip cannot overwrite the console script of the running process, but it can be renamed, so the worker moves it aside and pip writes a fresh one — and puts the old one back if the install fails. The running app keeps the version it started on, so restart to use the new one; *Restart now* does both at once, closing archeus and bringing it back on the new version. *Install on quit* is the same worker told to wait for archeus to exit first. A git checkout is told to `git pull` instead of having a release installed over it.
