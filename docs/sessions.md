...
- **Self-update** — archeus checks PyPI for a newer release and says so in the banner. `⚙ Updates` (and the GUI's Plugins page) offers the upgrade; `Settings → Updates` switches between *tell me*, *install on quit* and *off*. The install is staged into a background worker with no window of its own: it waits for archeus to exit — pip cannot rewrite the console script it is running from — installs, and then raises a desktop notification. *Restart now* does all of that at once, closing archeus and bringing it back on the new version. A git checkout is told to `git pull` instead of having a release installed over it.
