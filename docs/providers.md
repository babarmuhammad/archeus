...
anything "OpenAI-compatible" — is reachable through archeus's own **translating gateway**.
Turn it on in the same settings card: set the gateway target to the OpenAI-shaped URL and
archeus runs a loopback proxy that speaks Anthropic Messages to `claude` and OpenAI Chat
...
- **`count_tokens`** is answered with the same `chars//4` estimate archeus uses elsewhere.
...
1. ⚙ Settings → **Model provider** → **Backend**:
   ...
   - **OmniRoute** — archeus starts the daemon for you.
...
archeus checks the backend is reachable **before** the session opens. A launch that
...
Separately from your sessions, archeus makes its own headless `claude -p` calls — memory
...
**Run archeus's own calls here too** switches them over. They then ask for the model id above
...
These are the cheapest calls archeus makes and the best fit for a local model: short prompts,
...
these are archeus bugs — they are properties of Claude Code's client or of Anthropic's
own infrastructure.

| | What happens | What archeus does |
|---|---|---|
...
- **Q4_K_M is the floor for tool calling.** Q3 and below breaks the tool-call format before
  it visibly hurts chat quality, which makes it look like a archeus bug when it is not.
...
Separately, and this one is not ambiguous: **archeus never sends your Claude subscription
