---
description: >-
  Bulk-install the awesome-claude-code-subagents catalog into archeus's agent library, or
  add a single agent, without bloating every Claude Code session.
---

# Installing the agent library

The **⚙ Agents** screen reads `~/.claude/archeus-agents/<category>/*.md`. To bulk-install
...
!!! info "Credit"
    ...
    **[VoltAgent](https://github.com/VoltAgent)** —
    ...
    archeus only mirrors it into the library; all credit for the agents goes to the
...
$lib  = "$env:USERPROFILE\.claude\archeus-agents"
...
curl -sL https://raw.githubusercontent.com/VoltAgent/awesome-claude-code-subagents/main/categories/09-meta-orchestration/agent-installer.md \
  -o "$USERPROFILE/.claude/archeus-agents/09-meta-orchestration/agent-installer.md"
```

These land in the library (not `~/.claude/agents/`), so they don't bloat every Claude
session — archeus copies only the ones you select for a project into that project's
