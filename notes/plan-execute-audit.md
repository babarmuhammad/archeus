...
- **Build launch args**: `build_exec_launch(project_path, proj_folder, task, exec_model, prov_env=None, cfgdir='')` — returns `(args, env)` tuple; this is what the GUI also calls
...
- **Council**: `optimize_plan_council(task, plan, cwd, models=None, prov_env=None)` — fans draft out to N extra models for critique, then synthesizes
