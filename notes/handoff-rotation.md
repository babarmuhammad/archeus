...
- `tools/smoke_gui.py` — a `/api/rotate/state` fixture, `has_active` added to the
  `/api/accounts` stub, and four checks. No other track touches this file. It was worth it:
  the stub answers an unknown route with `{}` and a 200, so the card was rendering
  `Math.round(undefined)` as `NaN%`, and the missing `has_active` meant both the per-row
  quota ring and the rotation card were invisible to every audit this repo has.

Nothing else outside the fence. `CHANGELOG.md` untouched; no screenshots re-shot —
`tools/shot_gui.py` was run with a scratch output directory and no `--docs`, so the audit
ran (clean at 1280/1920/2560 across every skin, 864 containers probed) without rewriting a
single published image.
